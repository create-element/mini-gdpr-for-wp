...
All hooks use the `mwg_` prefix by convention.
...
|-----|------|----------|-------------|
| `handle` | string | Yes | WordPress script handle to register as blockable. Use lowercase with hyphens. |
| `description` | string | Yes | Human-readable name shown in the "Cookie Information" info overlay. |
| `sdk_url` | string | Yes | Full URL of the tracker SDK. Loaded by `loadCustomTrackers()` after consent. |
| `pattern` | string | No | Regex pattern (with delimiters) for server-side script blocking. |
| `field` | string | No | Which field to match the pattern against: `'src'` (default) or `'outerhtml'`. |
| `can_defer` | bool | No | If `true`, the script tag is suppressed pre-consent. Default `false`. |

See also: [Tracker Registration API](tracker-registration-api.md) for a full walkthrough.
...
### `mwg_myaccount_priority`

Change the hook priority at which the GDPR acceptance form is injected into the WooCommerce MyAccount page.

> **Note:** The previous filter name `pp_mwg_myaccount_priority` is deprecated since v2.0.0. It still works but will log a deprecation notice. Use `mwg_myaccount_priority` instead.

```php
add_filter( 'mwg_myaccount_priority', function ( int $priority ) : int {
...
### `mwg_enable_gdpr_registration_validation`
...
> **Note:** The previous filter name `pp_mwg_enable_gdpr_registration_validation` is deprecated since v2.0.0. It still works but will log a deprecation notice. Use `mwg_enable_gdpr_registration_validation` instead.

```php
// Disable GDPR validation for B2B registrations
add_filter( 'mwg_enable_gdpr_registration_validation', function ( bool $enabled ) : bool {
...
### `mwg_your_email_tag_name`

Customize the Contact Form 7 field tag name used to identify the email field when installing consent checkboxes.

> **Note:** The previous filter name `pp_mwg_your_email_tag_name` is deprecated since v2.0.0. It still works but will log a deprecation notice. Use `mwg_your_email_tag_name` instead.

```php
add_filter( 'mwg_your_email_tag_name', function ( string $tag ) : string {
...
The following `pp_mwg_*` filter names are deprecated since v2.0.0. They continue to work via a compatibility bridge, but using them will log a deprecation notice to the PHP error log. Migrate to the `mwg_*` equivalents listed above.

| Deprecated filter | Replacement |
|---|---|
| `pp_mwg_myaccount_priority` | `mwg_myaccount_priority` |
| `pp_mwg_enable_gdpr_registration_validation` | `mwg_enable_gdpr_registration_validation` |
| `pp_mwg_your_email_tag_name` | `mwg_your_email_tag_name` |
...
| `mwg_myaccount_priority` | Filter | 2.0.0 | WC MyAccount injection priority |
| `mwg_enable_gdpr_registration_validation` | Filter | 2.0.0 | Toggle WC registration validation |
| `mwg_your_email_tag_name` | Filter | 2.0.0 | CF7 email field tag name override |

---

## See Also

- [Tracker Registration API](tracker-registration-api.md) — Full tracker registration walkthrough
- [Developer Guide](developer-guide.md) — Architecture, class reference, coding standards
