# Migration Guide: v1.4.3 to v2.0.0
...
- **Deactivate, upload v2.0.0, activate** is the full upgrade procedure
...
- Three `pp_mwg_*` filters have been renamed to `mwg_*` (old names still work via deprecation bridge)
...
This is required for GDPR compliance. Users can now explicitly reject tracking, and a floating cookie button appears afterward so they can change their decision later.
...
A new `mwg_register_tracker` filter lets developers register arbitrary third-party trackers. See [hooks-and-filters.md](hooks-and-filters.md) and [tracker-registration-api.md](tracker-registration-api.md).

---

### 6. JavaScript Modernization (ES6+)

All JavaScript has been rewritten in ES6+ (no jQuery). The plugin no longer enqueues jQuery as a dependency. Minified assets are built with Terser (52-67% size reduction).
...
- Consent popup is a proper `role="dialog"` with `aria-modal`, `aria-live`, and `aria-describedby`
...
- `wp_kses_post()` used for consent message output
...
```php
// Old — pp-core functions (no longer available)
$settings = \PP\Core\get_settings( 'mini-wp-gdpr' );

// New — native WordPress Options API
...
### PHP: `pp_mwg_*` Filters Renamed

Three filters have been renamed from `pp_mwg_*` to `mwg_*`. The old names continue to work via a deprecation bridge, but will log a notice to the PHP error log:
...
Update your `add_filter()` calls to use the new names at your convenience.
...
The `pp-assets/` admin stylesheet has been replaced with the plugin's own `assets/admin/mwg-admin.css`.
...
In v1.x, `mini-gdpr.js` listed jQuery as a dependency. In v2.0.0, jQuery is no longer enqueued by this plugin.

If your code relied on this, add an explicit jQuery dependency to your own script:
```php
...
|---------------|------------------|
| `mgwAcceptCookies()` | *(called internally only)* |
...
All settings are stored using the same `wp_options` keys as v1.4.3:
...
|------------|------|---------|-------------|
| `mwg_consent_accept_text` | string | "Accept" | Accept button label |
| `mwg_consent_reject_text` | string | "Reject" | Reject button label |
| `mwg_consent_info_btn_text` | string | "info..." | Info button label |
...
- [ ] After upgrade: test Accept flow (trackers load, popup hides, cookie button appears)
- [ ] After upgrade: test Reject flow (no trackers load, popup hides, cookie button appears)
- [ ] After upgrade: test cookie button reopens the popup
- [ ] After upgrade: check WordPress error log is clean
- [ ] After upgrade: verify tracked user consent data is intact
...
- **GitHub Issues:** [github.com/create-element/mini-gdpr-for-wp/issues](https://github.com/create-element/mini-gdpr-for-wp/issues)
