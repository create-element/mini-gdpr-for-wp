...
For a structured migration guide see [migration-guide.md](migration-guide.md).
...
v2.0.0 requires **PHP 8.0+** and **WordPress 6.4+**. If your site is running an older environment you should update PHP/WordPress before upgrading the plugin.
...
The Reject button is a core GDPR compliance feature and cannot be hidden from the settings panel. If you need to remove it for a specific reason, override the CSS to hide `#mgwcsCntr .mgw-reject`.
...
Only if you were relying on the plugin to enqueue jQuery as a side-effect. If you see jQuery-related errors in the browser console after upgrading, verify that your own theme or plugin enqueues jQuery explicitly via `wp_enqueue_script( 'jquery' )`.
...
|---|---|
| `pp_core_is_mini_gdpr_enabled()` | `mwg_has_user_accepted_privacy_policy()` |
| `pp_core_get_date_time_now_h()` | PHP's `date_i18n()` or `current_time()` |
...
The `Component` class is now part of Mini WP GDPR itself (`includes/class-component.php`, namespace `Mini_Wp_Gdpr`). If you were extending pp-core's `PP_Framework\Component`, update your code:
...
2. Verify your GA Tracking ID is correct in **Settings > Mini WP GDPR > Google Analytics**.
3. Make sure "Enable GA Consent Mode v2" is checked if you want the consent signal sent.
4. Hard-refresh (Ctrl+Shift+R) to clear any cached decisions.

### Facebook Pixel isn't firing after acceptance.

The Pixel SDK is delay-loaded. After acceptance, the plugin grants the FB Consent API, loads `fbevents.js` dynamically, then calls `fbq('init', ...)` and `fbq('track', 'PageView')`. If the Pixel is still not firing:

1. Confirm your Pixel ID is entered in settings.
...
The popup layout changed from two buttons to three. Update your CSS to also handle `.mgw-reject`:

```css
#mgwcsCntr .mgw-accept { }  /* Accept button */
#mgwcsCntr .mgw-reject { }  /* Reject button (new in v2.0.0) */
#mgwcsCntr .mgw-info { }    /* "More info" / overlay button */
```
...
Check that the WooCommerce registration consent option is enabled in the plugin settings. v2.0.0 changed nothing about the WooCommerce integration logic; if the checkbox was showing before it should continue showing.

### The Contact Form 7 consent checkbox was removed from my form.

Re-open *Settings > Mini WP GDPR > Contact Form 7* and click *Install consent box* for the relevant form. The installer is idempotent — it will not duplicate the checkbox if it is already present.
